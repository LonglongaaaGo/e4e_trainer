...
class pSp(nn.Module):
    ...
    def load_weights(self):
        if self.opts.checkpoint_path is not None:
            ...
            if self.opts.multi_modal == False:
                self.encoder.load_state_dict(get_keys(ckpt, 'encoder'), strict=True)

            else:
                model_dict = self.encoder.state_dict()
                pretrained_dict = {k: v for k, v in ckpt["state_dict"].items() if (k in model_dict and ('input_layer.0' not in k))}
            ...
            if self.opts.multi_modal == False:  # if or not use more than three channels
                self.encoder.load_state_dict(encoder_ckpt, strict=False)
            else:
                model_dict = self.encoder.state_dict()
                pretrained_dict = {k: v for k, v in encoder_ckpt.items() if (k in model_dict and ('input_layer.0' not in k))}
                model_dict.update(pretrained_dict)
                self.encoder.load_state_dict(model_dict)
