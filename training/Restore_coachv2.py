...
from datasets.images_dataset import ImageFolder_restore_free_form
...
v2
加入了 更复杂的dataloader
+ 对 dataloader 进行了优化 加入haze 加入 灰度图处理， 加入free-form fusion,
( 加入随机偏移， )暂时去掉了，因为这不符合styleGAN的训练形式
"""
...
class Coach:
    ...
    def train(self):
        ...
        while self.global_step < self.opts.max_steps:
            for batch_idx, batch in enumerate(self.train_dataloader):
                ...
                low_img1, low_img2, real_img = batch
                #
                mask_01 = mat_256_mask(real_img.shape[0], self.opts.size,self.device)
                real_img = real_img.to(self.device).to(torch.float32) * 2.0 - 1
                low_img1 = low_img1.to(self.device).to(torch.float32) * 2.0 - 1
                low_img2 = low_img2.to(self.device).to(torch.float32) * 2.0 - 1
                low_img = low_img1 * mask_01 + low_img2 * (1 - mask_01)

                real_img_ori = real_img.clone()

                #
                batch = (low_img, real_img_ori)
    ...
    def configure_datasets(self):
        ...
        train_dataset = ImageFolder_restore_free_form(self.opts.img_path, transform=transforms_dict['transform_test'],im_size=(self.opts.size,self.opts.size))
        test_dataset = ImageFolder_restore_free_form(self.opts.img_test_path, transform=transforms_dict['transform_test'],im_size=(self.opts.size,self.opts.size))
