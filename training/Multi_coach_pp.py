...
from datasets.images_dataset import MultiImagesDataset
...
from training.Multi_coachv3 import random_sparse_sample_batch
...
class Coach:
    ...
    def train(self):
        ...
        while self.global_step < self.opts.max_steps:
            for batch_idx, batch in enumerate(self.train_dataloader):
                ...
                mask_01 = co_mod_mask_only(real_img.shape[0], im_size=self.opts.size)

                domin_num = random.randint(0,3)
                if domin_num==0:
                    real_img = mask_01*real_img
                else:
                    real_img = random_sparse_sample_batch(real_img)
                #
                if domin_num==1:
                    sketches_img = sketches_img*mask_01
                else:
                    sketches_img = random_sparse_sample_batch(sketches_img)
                #
                if domin_num==2:
                    semantic_map_nd = semantic_map_nd*mask_01
                    semantic_img_color = semantic_img_color* mask_01
                else:
                    semantic_map_nd,sparse_mask = random_sparse_sample_batch(semantic_map_nd,return_mask=True)
                    semantic_img_color = semantic_img_color* sparse_mask
                #
                if domin_num==3:
                    super_color = super_color*mask_01
                else:
                    super_color = random_sparse_sample_batch(super_color)
    ...
    def configure_datasets(self):
        ...
        train_dataset = MultiImagesDataset(
                                    img_root = self.opts.img_path,
        ...
        test_dataset = MultiImagesDataset(img_root = self.opts.img_test_path,
