...
from datasets.images_dataset import MaskImageDataset
...
color_policy = "translation,color"
# trans_policy = "reflect_translation,dis_scale"
trans_policy = "dis_scale"
...
class Coach:
    ...
    def train(self):
        ...
        while self.global_step < self.opts.max_steps:
            for batch_idx, batch in enumerate(self.train_dataloader):
                ...
                real_img,conditional_img = batch

                if self.opts.dataset_type == "cars_encode":
                    trans_policy = "reflect_translation,dis_scale"
                real_img,conditional_img = DiffAugment_withsame_trans(real_img,conditional_img, policy=trans_policy)

                real_img = real_img.to(self.device)
                # real_img = DiffAugment(real_img.clone(), policy=policy)  # for pose guidance

                conditional_img = conditional_img.to(self.device)
                ...
                if random.random() <= 0.5:  #
                    mask_01 = co_mod_mask_only(conditional_img.shape[0], im_size=self.opts.size)
                    if self.opts.dataset_type == "cars_encode":
                        mask_01 = mask_01[:, :, 32:224, :]
                    mask_01 = 1.0 - mask_01
                    conditional_img = conditional_img * mask_01
                ...
                gin = torch.cat([conditional_img], dim=1)
                ...
                if self.global_step % self.opts.image_interval == 0 or (
                        self.global_step < 1000 and self.global_step % 5 == 0):
                    ...
                    self.parse_and_log_images(id_logs, conditional_img, y, y_hat, title='images/train')
    ...
    def validate(self):
        ...
        for batch_idx, batch in enumerate(self.test_dataloader):
            ...
            conditional_img = conditional_img * (1.0-mask_01)
            # sketches_img = sketches_img * mask_01
            # semantic_map_nd = semantic_map_nd * mask_01
            # semantic_img_color = semantic_img_color * mask_01
            # super_color = super_color * mask_01
            ...
            # Logging related
            if batch_idx%100 == 0:
                self.parse_and_log_images(id_logs,conditional_img, y, y_hat,
    ...
    def configure_datasets(self):
        ...
        train_dataset = MaskImageDataset(
                                    img_root = self.opts.img_path,
                                    condition_root=self.opts.condition_path,
                                    target_transform=transforms_dict['transform_test'],
                                    opts=self.opts,
                                    im_size= (self.opts.size,self.opts.size),
                                    condition_type= self.opts.condition_type,
        )

        test_dataset = MaskImageDataset(img_root = self.opts.img_test_path,
                                        condition_root=self.opts.condition_test_path,
                                        target_transform=transforms_dict['transform_test'],
                                      opts=self.opts,
                                      im_size=(self.opts.size,self.opts.size),
                                      condition_type=self.opts.condition_type,
    ...
    def parse_and_log_images(self, id_logs,real_img, y, y_hat, title, subscript=None, display_count=2):
        im_data = []
    ...
    def log_images(self, name, im_data, subscript=None, log_latest=False):
        fig = common.vis_mask_faces(im_data)
